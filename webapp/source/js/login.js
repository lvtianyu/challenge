...
function doWxLogin(){
    ...
    $.ajax({
        ...
        url: nodeurl,
        data: {
            action: "user/getWechatLoginParams.do",
        },
        dataType: dataType,
...
function handleResult(json2){
    ...
    //    \"values\":{
    //    UID:\"fsfsfa\"
    //    url:\"http://tdw7lxraxf.proxy.qqbrowser.cc/eyun_challenge_sv/challenge/user/loginByWechat.do\"
    //}";
    //var json = eval("(" + demostr + ")");
    var result = json2.result;
    var valueobj = json2.values;
    var url = valueobj.url;
    //alert(url);
    ...
    var tpl = "";
    tpl += "<h1>uid: " + uid + "<br>";
    tpl += "<a id='wxdl' href='"+url+"'>"+"点击登录"+"</a><br>";
    tpl += "<a href=\"javascript: checkLogin(\'"+uid+"\')\">验证登录</a></h1>";
...
function checkLogin(uid){
    ...
    $.ajax({
        ...
        url: nodeurl,
        data: {
            action: "user/verifyWechatUser.do",
            UID: uid,
        },
        dataType: dataType,
        beforeSend: function(XMLHttpRequest) {
...
function handleResult2(json){
    var result = json.result;
    var valueobj = json.values;
    var tpl = "";
    if("ok"==result) {
        tpl += "userName: " + valueobj.userName;
    }else{
        tpl += "验证失败 " + json.errormsg;
    }
